...
public class DataSourceService {
    ...
    MessageResponse queryContactListByUserId(String userId, int limit, int offset, String uploadTime) {
        ...
        try {
            ...
            String totalSql;
            if (!uploadTime.equals("")) {
                totalSql = "SELECT COUNT(*) AS \"total\" FROM " + contactTable + " WHERE \"user_id\" LIKE ? AND \"upload_time\" > ?";
            } else {
                totalSql = "SELECT COUNT(*) AS \"total\" FROM " + contactTable + " WHERE \"user_id\" LIKE ?";
            }
    ...
    MessageResponse clearRegisterStatus(String phone) {
        long startTime = System.nanoTime();
        MessageResponse response = new MessageResponse();
        String sql;
        String message;
        ...
        try {
            if (connection == null) {
                message = "ClearRegisterStatus: connection is null";
                logger.error(message);
                response.setCode(10001);
                response.setMessage(message);
                return response;
            ...
            if (phone == null || phone.trim().length() <= 0) {
                message = "ClearRegisterStatus: no phone";
                logger.error(message);
                response.setCode(10002);
                response.setMessage(message);
                return response;
            ...
            logger.info(String.format("ClearRegisterStatus: uidListSize=%d, time=%d", uidList.size(), (System.nanoTime() - startTime) / 1000000));
            ...
            try (PreparedStatement statement = connection.prepareStatement(upsert)) {
                while (batchSize < size) {
                    String rowKey = uidList.get(batchSize);
                    statement.setString(1, rowKey);
            ...
            long estimatedTime = System.nanoTime() - clearStartTime;
            logger.info("ClearRegisterStatus time=" + estimatedTime / 1000000);

            connection.close();

        } catch (SQLException e) {
            e.printStackTrace();
            logger.error("ClearRegisterStatus: error while query ", e);
        }
